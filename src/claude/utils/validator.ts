...
export class Validator {
  private static readonly MODULE = "ClaudeValidator";

  static validateRequest(request: ClaudeRequestOptions): ValidationResult {
    const errors: string[] = [];
    ...
    if (!request.messages || !Array.isArray(request.messages)) {
      ...
      request.messages.forEach((msg, index) => {
        ...
        if (
          ...
          errors.push(
            `Message ${index}: Content must be a non-empty string or text blocks`,
          );
          ...
          const hasText = msg.content.some(
            (part) => part && part.type === "text" && part.text?.length,
          );
          if (!hasText) {
            errors.push(
              `Message ${index}: Content blocks must include at least one text part`,
            );
          }
        }
      });
